...
export default async function PersonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  else { try { profile = await readProfile(id); } catch (e) { return <div className="page"><Header title="Profile" /><p className="notice error">{e instanceof Error ? e.message : "This profile is unavailable."}</p></div>; } const results = await Promise.allSettled([recentPublic(profile.person.id, "Block"), recentPublic(profile.person.id, "Channel")]); if (results[0].status === "fulfilled") blocks = results[0].value; if (results[1].status === "fulfilled") channels = results[1].value; if (results.some(r => r.status === "rejected")) recentError = "Recent items could not be loaded from Are.na. Try again shortly."; }
  const session = await getSession(); const photo = safeUrl(profile.photo?.image?.medium?.src || profile.photo?.image?.src);
  return <div className="page"><Header title={profile.person.name} titleHref={`https://www.are.na/${profile.person.slug}`} actions={<>{session?.person?.id === profile.person.id ? <Link className="button" href="/profile/edit">Edit profile</Link> : <Link className="button" href={`/connections/new?person=${profile.person.id}&profile=${profile.channel.id}`}>Connect →</Link>}</>} />
