...
import { rememberConversation } from "@/lib/store";
export async function POST(request: NextRequest) {
  try { assertWrite(request); const session = await requireSession(); const { url } = z.object({ url: z.string().max(1000) }).parse(await request.json()); const ref = parseArenaUrl(url); if (ref.type !== "Channel") throw new Error("Choose a private conversation channel."); const channel = await new ArenaClient(session.token).item("Channel", ref.id); const people = assertParticipant(channel, session.person.id); rememberConversation(people[0].id, people[1].id, channel.id); return privateJson({ channelId: channel.id }); } catch (e) { return apiError(e); }
}
