...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  } catch (e) { return apiError(e, { route: "/api/conversations/[id]", method: "GET" }); }
}
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try { assertWrite(request); const session = await requireSession(); const { id } = await context.params; if (!/^\d+$/.test(id)) throw new Error("Invalid channel."); return privateJson(await sendMessage(new ArenaClient(session.token), session.person, Number(id), await request.json())); } catch (e) { return apiError(e, { route: "/api/conversations/[id]", method: "POST" }); }
}
