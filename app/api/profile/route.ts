...
export async function PATCH(request: NextRequest) { try { assertWrite(request); const session = await requireSession(); const { z } = await import("zod"); const body = z.object({ channelId: z.number().int().positive(), hidden: z.boolean() }).strict().parse(await request.json()); const { setProfileHidden } = await import("@/lib/profiles"); return privateJson(await setProfileHidden(session.person, session.token, body.channelId, body.hidden)); } catch (e) { return apiError(e); } }
