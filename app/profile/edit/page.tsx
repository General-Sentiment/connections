...
import { SampleAwareProfileHeader } from "@/components/dev-debug";
...
export default async function EditProfile() {
  ...
  if (session?.person && !demo) {
    ...
    } catch (e) { logServerError(e, { event: "page.load_failed", route: "/profile/edit", method: "GET" }); error = e instanceof Error ? e.message : "Could not load your profile."; }
  }
  return <div className="page"><SampleAwareProfileHeader hideArena={!profile} arenaHref={profile ? arenaUrl(profile.channel) : undefined} parent={profile ? "Edit" : "Create"} title={session?.person?.name || demoPeople[0].name} titleHref={`https://www.are.na/${encodeURIComponent(session?.person?.slug || demoPeople[0].slug)}`} />{error ? <p className="notice error">{error}</p> : !demo && !session?.person ? <div className="notice"><p>Log in with Are.na to add yourself. Logging in does not publish a profile.</p><Link className="button" href="/api/auth/login?next=/profile/edit">Log in with Are.na →</Link></div> : <ProfileForm key={`${demo ? "demo" : "live"}:${session?.person?.id || demoPeople[0].id}:${profile?.channel.id || "create"}`} person={session?.person || demoPeople[0]} profile={profile} demo={demo} />}{demo && <p className="preview-note">Preview · Changes are not published.</p>}</div>;
}
