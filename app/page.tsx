...
export default async function Directory({ searchParams }: { searchParams: Promise<{ page?: string; error?: string; order?: string; seed?: string; view?: string; open_to?: string }> }) {
  ...
  const pageHref = (number: number) => `/?view=${view}&order=${order}&page=${number}${filter ? `&open_to=${filter}` : ""}${order === "random" ? `&seed=${seed}` : ""}`;
  ...
  if (demo) items = demoProfiles.map(p => p.channel);
    ...
    if (filter) {
      const source = order === "random" ? await directoryClient("") : client;
      ...
      result = { data: await allItems(source, path), meta: { current_page: 1, next_page: null, has_more_pages: false, total_count: 0 } };
  ...
  let about = filter ? undefined : items.find(item => item.type === "Text" && item.title === "About");
  if (!filter && !demo && !about && aboutBlockId()) {
  ...
  return <div className="page directory-page homepage"><DirectoryHeader active="profiles" />
    <MobileDirectoryControls><div className="info-grid"><section><h2 className="info-title">About</h2><p>Meet people through their collections on Are.na.</p>{session?.person && <form className="about-logout" action="/api/auth/logout" method="post"><button className="quiet">Log out</button></form>}</section><section><h2 className="info-title">View</h2><DirectoryView view={view} /></section><section><h2 className="info-title">Order</h2><DirectoryOrder order={order} seed={seed} /></section><section><h2 className="info-title">Open to</h2><DirectoryFilter value={filter || "all"} /></section></div></MobileDirectoryControls>
