...
export function ProfileForm({ person, profile, demo }: { person: Person; profile?: Profile; demo: boolean }) {
  ...
  const payload = () => ({ whoAreYou, lookingFor, details, selected: selected.map((x, index) => x && ({ id: x.id, prompt: prompts[index], description: selectedDescriptions[itemKey(x)] || "", type: x.type === "Channel" ? "Channel" as const : "Block" as const })).filter(Boolean), photoProof, photoKey, removePhoto, useAccountPhoto });
  useWebMCP([
  ...
  return <form className={`profile-editor${busy ? " is-saving" : ""}`} aria-busy={busy} onSubmit={e => { e.preventDefault(); void submit(); }}>
    <fieldset disabled={busy} className="form-fieldset"><div className="profile-fields" inert={busy}>
      ...
      <section><fieldset className="intentions"><legend>Open to</legend>{intentions.map(([value, label]) => <label key={value} className="check"><input type="checkbox" disabled={preview} checked={details.open_to.includes(value)} onChange={e => setDetails({ ...details, open_to: e.target.checked ? [...details.open_to, value] : details.open_to.filter(x => x !== value) })} />{label}</label>)}</fieldset>{preview ? <label className="field">City<span>{details.location.city}</span></label> : <LocationPicker value={details.location} onChange={location => setDetails({ ...details, location })} />}<fieldset className="intentions location-preference"><legend>Meeting in person</legend><label className="check"><input type="checkbox" disabled={preview} checked={details.local_only} onChange={e => setDetails({ ...details, local_only: e.target.checked })} />Only people in my area</label></fieldset></section>
      <section><h3 className="profile-question-label">Who are you?</h3><textarea aria-label="Who are you?" className="looking-input" placeholder="Tell people a little about yourself…" maxLength={5000} required value={whoAreYou} onChange={e => setWhoAreYou(e.target.value)} /></section>
      <section><h3 className="profile-question-label">What are you looking for?</h3>{preview ? <p className="preview-text">{lookingFor}</p> : <textarea aria-label="What are you looking for?" className="looking-input" placeholder="A conversation, a collaborator, something unexpected…" maxLength={5000} required value={lookingFor} onChange={e => setLookingFor(e.target.value)} />}</section>
</div>
      <section className="content-section">{promptError && <p role="alert" className="error">{promptError} <button type="button" className="quiet" onClick={() => setPromptRetry(value => value + 1)}>Retry prompts</button></p>}<div className="grid">{prompts.map((_, i) => <div key={selected[i] ? `${selected[i].type}:${selected[i].id}` : `empty:${i}`} data-sort-index={i} className={`selected-slot${dragging === i ? " sorting" : ""}${dropTarget === i && dragging !== i ? " sort-target" : ""}`} draggable={!preview && Boolean(selected[i])} onDragStart={e => { if ((e.target as HTMLElement).closest("textarea")) { e.preventDefault(); return; } setDragging(i); e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData("text/plain", String(i)); }} onDragOver={e => { if (dragging === undefined || !selected[i]) return; e.preventDefault(); e.dataTransfer.dropEffect = "move"; setDropTarget(i); }} onDrop={e => { e.preventDefault(); if (dragging !== undefined && selected[i]) reorder(dragging, i); }} onDragEnd={() => { setDragging(undefined); setDropTarget(undefined); }}><SelectionPrompt prompt={prompts[i]} onRemix={preview ? undefined : () => remix(i)} onRemove={!preview && prompts.length > 3 ? () => {
