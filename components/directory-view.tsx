...
export function DirectoryView({ view }: { view: "grid" | "table" }) {
  ...
  return <div className="view-options" role="group" aria-label="Directory view" aria-busy={pending}>
    ...
    {(["grid", "table"] as const).map(value => <button key={value} type="button" aria-pressed={active === value} onClick={() => startTransition(() => { setActive(value); const query = new URLSearchParams(params.toString()); query.set("view", value); router.push(`${pathname}?${query}`, { scroll: false }); })}>{value === "grid" ? "Grid" : "Table"}</button>)}
  </div>;
}
