...
import { saveProfile, assembleProfile } from "../lib/profiles";
...
beforeEach(() => {
  ...
  for (const table of ["profiles", "operations", "conversations", "locks"]) database().prepare(`DELETE FROM ${table}`).run();
  ...
  vi.stubGlobal("fetch", vi.fn(async (url: string, options: RequestInit = {}) => {
    ...
    if (path.startsWith("/v3/search")) return response({ data: [], meta: { next_page: null } });
    ...
    if (path.startsWith("/v3/groups/77")) return response({ can: { manage_members: true }, data: [], meta: { next_page: null } });
    ...
    if (path.startsWith("/v3/blocks/")) { const id = Number(path.split("/").pop()); const block = blocks.get(id); if (method === "PUT" && block) { block.title = body.title; if (body.content) block.content = { markdown: body.content, plain: body.content }; } return response(block); }
    ...
    if (path.startsWith("/v3/connections/")) {
      const index = contents.findIndex(x => x.connection?.id === Number(path.split("/").pop()));
      if (index >= 0 && method === "DELETE") contents.splice(index, 1);
      if (index >= 0 && method === "PUT" && body.position) { const [item] = contents.splice(index, 1); contents.splice(body.position - 1, 0, item); }
...
describe("profile writes", () => {
  it("stores a link, biography, description, details and three real connections, and reuses them on edit", async () => {
    vi.stubEnv("ARENA_GROUP_ID", "77"); vi.stubEnv("ARENA_DIRECTORY_CHANNEL", "999");
    ...
    expect(contents.filter(x => x.connection?.metadata?.role === "selected")).toHaveLength(3);
  ...
  it("keeps description blocks adjacent through edits, reordering and removal", async () => {
    ...
    expect(contents.slice(-6).map(x => x.title)).toEqual(["Image 3", "Image 3", "Image 2", "Image 2", "Image 1", "Image 1"]);
  ...
  it("rejects selections created by someone else before creating a profile", async () => {
    vi.stubEnv("ARENA_GROUP_ID", "77"); vi.stubEnv("ARENA_DIRECTORY_CHANNEL", "999"); blocks.get(2)!.user = bob;
